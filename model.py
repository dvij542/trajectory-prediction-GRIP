...
class Model(nn.Module):
	def __init__(self, in_channels, graph_args, edge_importance_weighting, **kwargs):
		...
		self.st_gcn_networks = nn.ModuleList((
			...
			Graph_Conv_Block(64, 64, kernel_size, 1, residual=True, **kwargs),
	...
	def forward(self, pra_x, pra_A, pra_pred_length, pra_teacher_forcing_ratio=0, pra_teacher_location=None):
		...
		hist_enc = self.leaky_relu(self.dyn_emb(hist_enc1.view(hist_enc1.shape[1],hist_enc1.shape[2])))
		hist_enc1 = self.leaky_relu(self.def_dyn_emb(hist_enc1.view(hist_enc1.shape[1],hist_enc1.shape[2])))
