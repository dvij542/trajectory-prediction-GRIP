...
def train_model(pra_model, pra_data_loader, pra_optimizer, pra_epoch_log):
	...
	for iteration, (_,ori_data, A, _) in enumerate(pra_data_loader):
		...
		for now_history_frames in range(6,min(7, data.shape[-2])):
			...
			output_mask = data[:,-1:,now_history_frames:,:]*(data[:,0:1,5:6,:]!=0).float().to(dev) * (((cat_mask==1)+(cat_mask==2)+(cat_mask==4))>0).float().to(dev)# (N, C, T, V)=(N, 1, 6, 120)
...
if __name__ == '__main__':
	...
	parser.add_argument("-n", "--no_of_epochs", default=50, help="No of epochs to run")
	...
	run_trainval(model, pra_traindata_path='train_data.pkl', pra_testdata_path='test_data.pkl', use_wandb=args.wandb)
