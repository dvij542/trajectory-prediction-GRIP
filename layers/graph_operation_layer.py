...
class ConvTemporalGraphical(nn.Module):
	...
	def forward(self, x, A):
		...
		for i in range(A.shape[0]):
			...
			edge_attr = torch.tensor([Ab[i, :7, index[0][m], index[1][m]] for m in range(index.shape[1])])
			# print(edge_attr.shape)
			# print("XXXXXXXXXXXXXX")
			data = Data(x =torch.transpose(x_reshaped[i].detach().cpu(),0,1), edge_index = index, edge_attr = edge_attr)
			datalist.append(data)
		train_loader = DataLoader(datalist, batch_size=x.shape[0], shuffle=True)
		...
		new_a = pyg_utils.to_dense_adj(edge_index = data1.edge_index, batch = data1.batch, edge_attr=data1.edge_attr, max_num_nodes = 400)
